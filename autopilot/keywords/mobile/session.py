...
from ...mobile.adb import run_adb, adb_shell, require_android_package, require_url_scheme
...
def _parse_apk_kw(path: str):
    ...
    from ...mobile.apk import parse_apk  # 延迟：仅 Android 安装解析
...
def _parse_ipa_kw(path: str):
    from ...mobile.ipa import parse_ipa  # 延迟：仅 iOS 安装解析
    try:
...
def _ios_app_installed(bundle_id: str, udid: str = "") -> bool:
    if not bundle_id:
        return False
    try:
        from pymobiledevice3.lockdown import create_using_usbmux  # 延迟：可选 pmd3 extra
...
def ios_install_app(ipa_path: str, udid: str = "",
    ...
    from ...mobile.ipa import ipa_precheck
...
def intent_to_mini_program(ctx: ExecutionContext, urlPath: str = "", **_kw) -> None:
    ...
    serial = _serial(ctx)
    try:
        safe = require_url_scheme(urlPath)
