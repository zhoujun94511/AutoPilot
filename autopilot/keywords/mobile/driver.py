...
def default_appium_factory(os_type: str, package: str, activity: str, server: str,
    ...
    try:
        ...
        raise KeywordError(
            ...
            "请 pip install autopilot[mobile] 或 pip install Appium-Python-Client"
        ) from e
...
def mirror_control_sink(mgr: AppiumManager, drv: Any) -> Any | None:
    """镜像控制汇：按 backend 显式分支，两条路径互不影响。

    - ``backend == "wda"``：WDA-direct（Windows/Linux 主路径）→ ``WdaControlSink``
    - ``backend == "appium"``：Mac iOS Appium 检视 → ``AppiumControlSink``
    """
    if getattr(mgr, "platform", "") != "ios":
        return None
    backend = (getattr(mgr, "backend", "") or "").lower()
    if backend == "wda":
        client = _ios_wda_client_from_driver(drv)
        if client is None:
            return None
        from ...inspector.stream.control import WdaControlSink
        return WdaControlSink(client)
    if backend == "appium":
        from ...inspector.stream.control import AppiumControlSink

        def _driver_provider():
            d = mgr.optional_driver()
            if d is None:
                raise KeywordError("移动端会话未创建，请先刷新检视快照")
            return d

        return AppiumControlSink(_driver_provider)
    return None
...
def try_ios_alert_click(ctx: ExecutionContext, locator, timeout_ms: int,
    ...
    前置条件：session 不得绑 bundleId，否则 Alert 不在 page_source（真机实测结论）。
