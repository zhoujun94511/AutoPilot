...
def _install_pinned_build(app_path: str, udids: list[str], platform: str) -> str | None:
    ...
    for uid in targets:
        try:
            if ios:
                from autopilot.keywords.mobile.session import ios_install_app

                ios_install_app(app_path, udid=uid)
            else:
                from autopilot.mobile.xapk import install_android_package
...
def execute_job(
    ...
    *,
    ...
    from autopilot.engine import FaultStrategy, run_project_directory  # 延迟：仅真正执行 Job
    from autopilot.report import (
        ReportMeta,
        cases_from_suite,
        report_filename,
        write_report,
    ...
    def _run() -> JobResultIn:
        ...
        if plat == "web":
            ...
            from autopilot.keywords.http.env import apply_job_http_env
        ...
        handler = _ListHandler(str(job.id))
        ...
        try:
            ...
            try:
                from autopilot import __version__ as _ap_ver
