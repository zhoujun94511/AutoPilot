"""从 CLI / CI 触发 Platform 批跑 Job（B1-C）。"""
...
def build_job_body(args: argparse.Namespace) -> dict[str, Any]:
    """把 CLI 参数收成 Platform ``JobCreate`` 兼容 dict。"""
...
def _add_job_args(ap: argparse.ArgumentParser) -> None:
    ap.add_argument(
        ...
        help="Platform base URL（默认 MC_SERVER / 内置平台地址）",
    )
    ...
    auth.add_argument(
        ...
        help="用户 Bearer JWT（默认 MC_JWT；与 --token 二选一）",
    )
    ...
    ap.add_argument(
        "--project-dir",
        default="",
        help="Runner 本机工程路径（无制品时用；CI 通常用 --artifact-id）",
    )
    ap.add_argument(
        "--device-udids",
        default="",
        help="逗号分隔 UDID；空=任意空闲 Runner（若 Platform 未强制要求设备）",
    )
    ap.add_argument(
        "--entry-paths",
        default="",
        help="逗号分隔相对路径（.tc/.ts/.tp）；空=发现全部用例",
    )
    ...
    ap.add_argument(
        "--wait",
        action="store_true",
        help="创建后轮询直到终态（succeeded/failed/cancelled）",
    )
...
def _wait_job(client: MgmtClient, job_id: str, *, timeout: float, interval: float) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="触发 Platform 批跑 Job（CI / 运维脚本）"
    )
    _add_job_args(ap)
