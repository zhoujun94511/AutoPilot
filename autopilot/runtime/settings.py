...
def ui_theme() -> str:
    """界面主题偏好：system | light | dark。"""
    from ..ui.theme import DEFAULT_THEME, normalize_stored_theme  # 延迟：theme 顶栏 import settings

    value = str(load().get("ui_theme", DEFAULT_THEME) or DEFAULT_THEME).strip().lower()
    return normalize_stored_theme(value)


def set_ui_theme(theme: str) -> None:
    from ..ui.theme import normalize_stored_theme  # 延迟：theme 顶栏 import settings

    set_value("ui_theme", normalize_stored_theme(theme))
...
def http_env_profile() -> str:
    """HTTP 批跑默认 api_env.yaml profile；空/auto=用例内切换。"""
    value = str(load().get("http_env_profile", "") or "").strip()
    return "" if value.lower() == "auto" else value


def set_http_env_profile(profile: str) -> None:
    value = str(profile or "").strip()
    if value.lower() == "auto":
        value = ""
    set_value("http_env_profile", value)
...
def mc_server_url_stored() -> str:
    """用户显式写入 settings.json 的 Platform 地址（空表示用 env/默认）。"""
    return str(load().get("mc_server_url", "") or "").strip().rstrip("/")


def mc_server_url() -> str:
    """有效 Platform 根 URL（全 IDE 唯一网络入口）。"""
    from .platform_url import platform_base_url

    return platform_base_url(include_settings=True)


def set_mc_server_url(url: str) -> None:
    from .platform_deploy import platform_url_locked
...
def mc_bound_project_id(project_dir: str) -> str:
    """读取某个本地工程记住的 Platform 项目映射。"""
    key = _norm(os.path.abspath(project_dir or ""))
    raw = load().get("mc_project_bindings", {})
    if not key or not isinstance(raw, dict):
        return ""
    return str(raw.get(key) or "").strip()


def set_mc_bound_project_id(project_dir: str, project_id: str) -> None:
    """记住本地工程 → Platform 项目；空 project_id 表示删除映射。"""
    key = _norm(os.path.abspath(project_dir or ""))
    if not key:
        return
    data = load()
    raw = data.get("mc_project_bindings", {})
    bindings = dict(raw) if isinstance(raw, dict) else {}
    pid = str(project_id or "").strip()
    if pid:
        bindings[key] = pid
    else:
        bindings.pop(key, None)
    data["mc_project_bindings"] = bindings
    save(data)


def mc_save_sync_prompt() -> bool:
    """保存用例后是否提示「是否同步 Platform」（UX-P2-001）。"""
    raw = load().get("mc_save_sync_prompt", True)
    if isinstance(raw, bool):
        return raw
    return str(raw).strip().lower() not in ("0", "false", "no", "off")


def set_mc_save_sync_prompt(enabled: bool) -> None:
    set_value("mc_save_sync_prompt", bool(enabled))
