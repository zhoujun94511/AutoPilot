"""IDE 本机镜像用的 iOS 27+ CoreDevice HEVC 画面（主机不转码）。

只供 AutoPilot Inspector。设备通过 ``startmediastream`` 推 RTP/HEVC，
本模块开独立 Userspace RSD、解 RTP、产出访问单元，再由 PyAV 解码。
iOS 27.0 以下会返回 code 9021，调用方应留在 WDA MJPEG。不替换 macOS AVFoundation。

本文件不属于双仓同构执行核，不进入 ``ap/``。Platform 远控有自己的 Runner 实现。
