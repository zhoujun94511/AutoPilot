...
_URL_SCHEME_RE = re.compile(r"^[a-zA-Z][a-zA-Z0-9+\-.]*://[^\s'\"\\]+$")
...
def require_url_scheme(value: str, *, what: str = "urlPath") -> str:
    """校验 URL Scheme（``am start -d``）；配合 ``run_adb`` 参数列表，禁止引号/控制符。"""
    s = str(value or "").strip()
    if not s:
        raise ValueError(f"{what} 不能为空（AUD-2026-10）")
    if re.search(r"[\x00-\x1f'\"\\]", s):
        raise ValueError(f"{what} 含非法字符（AUD-2026-10）: {s[:80]!r}")
    if not _URL_SCHEME_RE.fullmatch(s):
        raise ValueError(f"{what} 不是合法 URL Scheme（AUD-2026-10）: {s[:80]!r}")
    return s
...
def run_adb(args: list[str], serial: str = "", timeout: int = 30) -> str:
    ...
    proc = _run(cmd, capture_output=True, timeout=timeout)
