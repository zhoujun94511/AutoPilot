...
from autopilot.runner.instance_lock import (
...
def test_lock_released_allows_reacquire(tmp_path):
    lock_dir = str(tmp_path / "locks")
    rid = "reacquire-me"
    first = RunnerInstanceLock(rid, lock_dir=lock_dir)
    first.acquire()
    first.release()
    second = RunnerInstanceLock(rid, lock_dir=lock_dir)
    second.acquire()
    assert second.held
    second.release()
...
def test_lock_path_stable_for_same_id(tmp_path):
    ...
    p1 = lock_path_for("MyHost/abc", lock_dir=lock_dir)
    p2 = lock_path_for("MyHost/abc", lock_dir=lock_dir)
    assert p1 == p2
    assert p1.parent == lock_dir


def test_run_forever_aborts_when_lock_held(tmp_path, monkeypatch):
    from autopilot.runner import agent as agent_mod
